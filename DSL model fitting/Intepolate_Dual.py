...
file = '3_output_Isotherm_pressure_new.xlsx'
...
file = '3_output_Isotherm_loading_new.xlsx'
Databook = pd.ExcelFile(file)
...
file = '3_output_Isotherm_pairs_new.xlsx'
Databook = pd.ExcelFile(file)
Original_Pairs = Databook.parse('Sheet1', header = 0, index_col = None)

Pairs = Original_Pairs.iloc[:, 6]
...
for i in range(5158, len(Load)):
    ...
    if Pairs[i] != 'hydrogen':
        fit_params.add('Q1', value = 5, min = 0.01, max = 20)
        fit_params.add('b1', value = 10, min = 0.001, max = 1000)
